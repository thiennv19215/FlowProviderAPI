...
class ProjectRegistry:
    ...
    def __init__(self):
        self._locks: weakref.WeakValueDictionary[tuple[str,str,str],asyncio.Lock]=weakref.WeakValueDictionary()

    def _lock_for(self,client_id:str,workspace_key:str,account_id:str)->asyncio.Lock:
        key=(client_id,workspace_key,account_id);lock=self._locks.get(key)
        if lock is None:
    ...
    async def get_or_create(self,db,*,client_id:str,workspace_key:str,account_id:str,sdk)->str:
        lock=self._lock_for(client_id,workspace_key,account_id)
        async with lock:
            ...
            db.commit();title=f"Provider {workspace_key}"[:120];result=await sdk.create_project(title)
