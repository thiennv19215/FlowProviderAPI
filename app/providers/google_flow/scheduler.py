...
def estimated_credit_cost(kind: str, payload: dict | None = None) -> int:
    payload=payload or {}
    if kind=="omni":return OMNI_CREDIT_COST.get(int(payload.get("duration",8)),30)
    return VIDEO_MIN_CREDITS.get(kind,0)


class GoogleFlowScheduler:
    """Google Flow account, slot, and credit reservation policy."""
    ...
    def _lock_account(db,account_id:str)->None:
        if db.bind and db.bind.dialect.name=="postgresql":
            db.execute(text("SELECT pg_advisory_xact_lock(hashtextextended(:key,0))"),{"key":f"provider-account:{account_id}"})
    ...
    def _ranked_candidates(self,db,*,kind:str,payload:dict|None,client_id:str|None,provider:str|None):
        ...
        for conn in self.bridge.ready_connections():
            ...
            candidates.append((0 if conn.id in sticky_accounts else 1,active,-available,conn.connected_at,conn.id))
        candidates.sort();return [item[4] for item in candidates]
