...
def resolve_paygate_tier(payload: dict) -> str | None:
    ...
    if tier in SUPPORTED_PAYGATE_TIERS:
        return tier
...
class ExtensionConnection:
    ...
    max_slots: int = 3
    max_image_slots: int = 4
    max_video_slots: int = 3
...
class FlowBridge:
    ...
    def __init__(
        self,
        *,
        flow_api_key: str | None,
        slot_capacity: int = 3,
        image_slot_capacity: int = 4,
        video_slot_capacity: int = 3,
        cooldown_seconds: int = 180,
    ):
        self.flow_api_key = flow_api_key
        self.slot_capacity = slot_capacity
        self.image_slot_capacity = image_slot_capacity
        self.video_slot_capacity = video_slot_capacity
        ...
        async def runner():
            try:
                ...
                while True:
                    ...
                    if self._refresh_tasks.get(connection_id) is not refresh_task:
                        return
                    conn = self.get(connection_id)
                    if not conn or not conn.flow_key or isinstance(conn.credits, int):
                        return
    ...
    def connections(self) -> list[ExtensionConnection]:
        return list(self._connections.values())

    def connected_count(self) -> int:
        return len(self._connections)

    def pending_count(self, connection_id: str) -> int:
        return sum(
            1 for _req_id, (_future, cid) in self._pending.items() if cid == connection_id
        )

    def get_connection_by_installation(self, installation_id: str) -> ExtensionConnection | None:
        cid = self._installation_to_id.get(installation_id)
    ...
    def register(self, ws: Any, hello: dict) -> ExtensionConnection:
        ...
        if prior:
            self.clear(connection_id=cid)
        conn = ExtensionConnection(
            id=cid,
            ws=ws,
            installation_id=installation,
            runtime_id=str(hello.get("runtimeId") or "chrome")[:40],
            profile_id=str(hello.get("profileId") or installation)[:128],
            profile_name=str(hello.get("profileName") or "Browser extension")[:160],
            max_slots=self.slot_capacity,
            max_image_slots=self.image_slot_capacity,
            max_video_slots=self.video_slot_capacity,
            extension_connection_id=str(hello.get("connectionId") or "")[:128] or None,
        )
        self._connections[cid] = conn
        self._installation_to_id[installation] = cid
        self._ws_to_id[id(ws)] = cid
        return conn

    def clear(self, ws: Any | None = None, *, connection_id: str | None = None) -> None:
        if ws is not None:
            connection_id = self._ws_to_id.get(id(ws))
        if not connection_id:
            return
        conn = self._connections.pop(connection_id, None)
        if not conn:
            return
        self._installation_to_id.pop(conn.installation_id, None)
        self._ws_to_id.pop(id(conn.ws), None)
    ...
    def get(self, connection_id: str) -> ExtensionConnection | None:
        return self._connections.get(connection_id)

    def mark_suspect(self, connection_id: str) -> ExtensionConnection | None:
        conn = self.get(connection_id)
        if conn and conn.suspect_since is None:
            conn.suspect_since = time.time()
        return conn
    ...
    def list_accounts(self) -> list[dict]:
        ...
        for c in sorted(self._connections.values(), key=lambda x: x.connected_at):
            cooldown = (
                max(0, int(c.cooldown_until - now))
                if c.cooldown_until and c.cooldown_until > now
                else 0
            )
            result.append({
                "id": c.id,
                ...
                "paygate_tier": c.paygate_tier,
                "sku": c.sku,
                "credits": c.credits,
                "slot_capacity": c.max_slots,
    ...
    def ready_connections(self, *, min_credits: int = 0) -> list[ExtensionConnection]:
        ...
        for c in self._connections.values():
            ...
            if c.cooldown_until and c.cooldown_until > now:
                continue
            if min_credits and (c.credits is None or c.credits < min_credits):
                continue
            out.append(c)
    ...
    def _invalidate_auth(self, conn: ExtensionConnection, reason: str | None = None) -> None:
        ...
        conn.sku = None
        conn.credits = None
        conn.last_error = (reason or "authentication_required")[:300]
    ...
    async def handle_message(self, data: dict, ws: Any) -> None:
        ...
        if msg_type == "flow_api_key" and conn:
            ...
            if (
                ...
                and all(ch.isalnum() or ch in "-_" for ch in api_key)
            ):
                changed = api_key != conn.flow_api_key
                conn.flow_api_key = api_key
                if conn.flow_key and changed:
                    self.schedule_account_refresh(conn.id)
            return
        ...
        if msg_type == "user_info" and conn:
            ...
            if isinstance(info, dict):
                conn.user_info = {
                    k: info.get(k)
                    for k in ("email", "name", "picture", "verified_email")
                    if k in info
                }
        ...
        if msg_type == "auth_sync_status" and conn:
            ...
            if status in {"needs_labs_sign_in", "signed_out", "auth_error"}:
                self._invalidate_auth(conn, str(data.get("reason") or status))
                await self._send_auth_ack(conn)
            return
        ...
        if req_id and req_id in self._pending:
            ...
            if not future.done():
                ...
                if data.get("error"):
                    if c:
                        c.failed_count += 1
                        c.last_error = str(data.get("error"))[:300]
                else:
                    if c:
                        c.success_count += 1
    ...
    async def refresh_account(self, connection_id: str) -> None:
        ...
        spec = {
            ...
            "method": "GET",
            "headers": {
                "authorization": f"Bearer {conn.flow_key}",
        ...
        if not isinstance(inner, dict) or inner.get("ok") is False:
            ...
            reason = (
                f"flow_credits_http_{status}"
                if isinstance(status, int)
                else "flow_credits_unavailable"
            )
        ...
        if isinstance(payload, dict):
            ...
            conn.credits = (
                payload.get("credits") if isinstance(payload.get("credits"), int) else None
            )
            conn.sku = (
                payload.get("sku") if isinstance(payload.get("sku"), str) else None
            )
        else:
    ...
    async def send_rpc(
        ...
        *,
        ...
        self._pending[req_id] = (future, connection_id)
        conn.request_count += 1
        try:
            await conn.ws.send(json.dumps({"id": req_id, "type": rpc_type, **params}))
            return await asyncio.wait_for(future, timeout=timeout or self.DEFAULT_TIMEOUT)
        except asyncio.TimeoutError:
            self._pending.pop(req_id, None)
            try:
                await conn.ws.send(json.dumps({"type": "CANCEL_RPC", "targetRequestId": req_id}))
            except Exception:
                pass
            conn.failed_count += 1
    ...
    async def api_request(
        ...
        *,
        ...
        if not conn:
            return {"error": "extension_disconnected"}
        bearer_resp = await self.send_rpc(
            connection_id, "GET_BEARER", {}, timeout=self.BEARER_TIMEOUT
        )
        bearer = bearer_resp.get("data") if isinstance(bearer_resp, dict) else None
        if not isinstance(bearer, str) or not bearer:
            self._invalidate_auth(
                conn, bearer_resp.get("error") if isinstance(bearer_resp, dict) else None
            )
            return {"error": bearer_resp.get("error") or "no_bearer_from_extension"}
        conn.flow_key = bearer
        final_body = copy.deepcopy(body)
        if captcha_action:
            tab_resp = await self.send_rpc(
                connection_id, "OPEN_FLOW_TAB", {"allowCreateHome": True}, timeout=self.TAB_TIMEOUT
            )
        ...
        fetch_headers["authorization"] = f"Bearer {bearer}"
        is_get = method.upper() in {"GET", "HEAD"}
        if is_get:
            fetch_headers = {
                k: v for k, v in fetch_headers.items() if k.lower() != "content-type"
            }
        else:
        ...
        return self._normalize_fetch(
            await self.send_rpc(
                connection_id,
                "SW_FETCH",
                {"spec": spec},
                timeout=(timeout or self.DEFAULT_TIMEOUT) + 5,
            ),
            "API",
        )
    ...
    async def trpc_request(
        ...
        *,
        ...
        if not bearer:
            ...
            return {"error": "no_bearer_from_extension"}
        tab_resp = await self.send_rpc(
            connection_id, "OPEN_FLOW_TAB", {"allowCreateHome": False}, timeout=self.TAB_TIMEOUT
        )
        tab_data = tab_resp.get("data") if isinstance(tab_resp, dict) else None
        tab_id = tab_data.get("tabId") if isinstance(tab_data, dict) else None
        if not tab_id:
        ...
        return self._normalize_fetch(
            await self.send_rpc(
                connection_id,
                "INJECT_PAGE_FETCH",
                {"tabId": tab_id, "spec": spec},
                timeout=timeout + 5,
            ),
            "TRPC",
            final_url_as_data=True,
        )

    @staticmethod
    def _normalize_fetch(
        response: dict, prefix: str, final_url_as_data: bool = False
    ) -> dict:
        if response.get("error"):
            return {"error": response["error"]}
        inner = response.get("data") or {}
        if not isinstance(inner, dict):
            return {"error": f"unexpected_{prefix.lower()}_response"}
        out = {"status": inner.get("status")}
        if isinstance(inner.get("headers"), dict):
            out["headers"] = {
                str(k).lower(): str(v) for k, v in inner["headers"].items()
            }
        if inner.get("data") is not None:
            out["data"] = inner["data"]
        elif isinstance(inner.get("text"), str):
            try:
                out["data"] = json.loads(inner["text"])
            except Exception:
                out["text"] = inner["text"]
        elif final_url_as_data and inner.get("finalUrl"):
            out["data"] = {"url": inner["finalUrl"]}
        if inner.get("ok") is False:
            out["error"] = inner.get("error") or f"{prefix}_{inner.get('status', '?')}"
        return out

    async def resolve_media_url(
        self, connection_id: str, media_id: str, *, thumbnail: bool = False
    ) -> str | None:
        query = {"name": media_id}
        if thumbnail:
            query["mediaUrlType"] = "MEDIA_URL_TYPE_THUMBNAIL"
        url = f"https://labs.google/fx/api/trpc/media.getMediaUrlRedirect?{urlencode(query)}"
        response = await self.send_rpc(
            connection_id,
            "SW_FETCH",
            {"spec": {"url": url, "method": "GET", "headers": {}, "responseType": "none", "timeoutMs": 30000}},
            timeout=35,
        )
        inner = response.get("data") if isinstance(response, dict) else None
        if not isinstance(inner, dict) or inner.get("ok") is False:
    ...
    async def download_media(self, connection_id: str, media_id: str) -> dict:
        ...
        if not url:
            return {"error": "media_url_unavailable"}
        try:
            parsed = urlparse(url)
        except Exception:
        ...
        if parsed.scheme != "https" or not any(
            hostname == host or hostname.endswith(f".{host}") for host in MEDIA_DOWNLOAD_HOSTS
        ):
        ...
        try:
            async with httpx.AsyncClient(timeout=60, follow_redirects=False) as client:
                async with client.stream("GET", url) as response:
                    ...
                    if response.url.scheme != "https" or not any(
                        final_host == host or final_host.endswith(f".{host}")
                        for host in MEDIA_DOWNLOAD_HOSTS
                    ):
                        return {"error": "media_download_redirect_not_allowed"}
                    if response.status_code >= 400:
                        return {
                            "error": f"media_download_http_{response.status_code}",
    ...
    def mark_provider_failure(
        self, connection_id: str, error: str, *, status_code: int | None = None
    ) -> None:
        conn = self.get(connection_id)
        if not conn:
            return
        text = error.lower()
        conn.last_error = error[:300]
        if (
            ...
            self._invalidate_auth(conn, error)
        if (
            status_code == 429
            or "429" in text
            ...
            conn.cooldown_until = time.time() + self.cooldown_seconds
            conn.cooldown_reason = "rate_limit" if "quota" not in text else "quota"
...
class BoundFlowClient:
    ...
    def _observe(self, result: dict) -> dict:
        ...
        if isinstance(result, dict) and (
            result.get("error") or (isinstance(status, int) and status >= 400)
        ):
    ...
    async def resolve_media_url(self, media_id: str, *, thumbnail: bool = False) -> str | None:
        return await self.bridge.resolve_media_url(
            self.connection_id, media_id, thumbnail=thumbnail
        )
