...
class FlowBridge:
    ...
    def register(self, ws: Any, hello: dict) -> ExtensionConnection:
        ...
        prior = self.get_connection_by_installation(installation)
        if prior:
            self.clear(connection_id=prior.id)
        cid = str(uuid.uuid4())
    ...
    async def _send_auth_ack(self, conn: ExtensionConnection) -> None:
        try:
            await conn.ws.send(json.dumps({"type":"auth_sync_ack","connectionId":conn.extension_connection_id or conn.id,"backendConnectionId":conn.id,"status":"synced" if conn.ready else "auth_syncing","email":conn.account_email,"credits":conn.credits,"busy":False,"activeSlots":0,"slotCapacity":conn.max_slots}))
    ...
    async def refresh_account(self, connection_id: str) -> None:
        ...
        response=await self.send_rpc(connection_id,"SW_FETCH",{"spec":spec},timeout=35);inner=response.get("data") if isinstance(response,dict) else None;payload=inner.get("data") if isinstance(inner,dict) else None
        if isinstance(payload,dict):
            tier=payload.get("userPaygateTier")
            if tier in {"PAYGATE_TIER_ONE","PAYGATE_TIER_TWO"}:conn.paygate_tier=tier
            if isinstance(payload.get("credits"),int):conn.credits=payload["credits"]
            if isinstance(payload.get("sku"),str):conn.sku=payload["sku"]
    ...
    async def api_request(self, connection_id: str, *, url: str, method: str = "POST", headers: dict | None = None, body: Any = None, captcha_action: str | None = None, timeout: float | None = None) -> dict:
        ...
        if not isinstance(bearer,str) or not bearer:return {"error":bearer_resp.get("error") or "no_bearer_from_extension"}
        ...
        response=await self.send_rpc(connection_id,"SW_FETCH",{"spec":spec},timeout=(timeout or self.DEFAULT_TIMEOUT)+5);return self._normalize_fetch(response,"API")
    ...
    async def trpc_request(self, connection_id: str, *, url: str, method: str = "GET", headers: dict | None = None, body: Any = None, timeout: float = 30, response_type: str | None = None) -> dict:
        ...
        is_get=method.upper()=="GET";fetch_headers=dict(headers or {})
        if bearer:fetch_headers["authorization"]=f"Bearer {bearer}"
        ...
        response=await self.send_rpc(connection_id,"INJECT_PAGE_FETCH",{"tabId":tab_id,"spec":spec},timeout=timeout+5);return self._normalize_fetch(response,"TRPC",final_url_as_data=True)
    ...
    async def resolve_media_url(self, connection_id: str, media_id: str) -> str | None:
        ...
        if isinstance(inner,dict) and isinstance(inner.get("finalUrl"),str):return inner["finalUrl"]
        return None
