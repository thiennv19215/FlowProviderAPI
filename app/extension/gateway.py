...
async def receive_json(ws: WebSocket, timeout: float):
    ...
    if not isinstance(data, dict):
        raise ValueError("object_frame_required")
    return data


async def heartbeat_loop(conn, bridge, manager, settings):
    interval = settings.extension_heartbeat_seconds
    grace = settings.extension_heartbeat_grace_seconds
    while bridge.get(conn.id) is conn:
        await asyncio.sleep(interval)
        if bridge.get(conn.id) is not conn:
            return
        response = await bridge.send_rpc(
            conn.id,
            "PING",
            {},
            timeout=min(10, max(3, grace)),
        )
        if not response.get("error"):
            bridge.mark_healthy(conn.id)
            manager.heartbeat(conn)
            continue
        current = bridge.mark_suspect(conn.id)
        if current is not conn:
            return
        manager.suspect(conn)
        marker = conn.suspect_since
        ...
        if conn.suspect_since is None or conn.suspect_since != marker:
            ...
            continue
        try:
            await conn.ws.close(4408, "extension heartbeat timeout")
        except Exception:
            pass
        return
...
def _capabilities_from_hello(hello: dict) -> set[str] | None:
    """Validate explicit capabilities while preserving legacy v7 connectors.

    Older protocol-v7 extensions did not advertise capabilities, so an absent
    field remains compatible. Once a connector advertises the field, however,
    it must be well formed and include the executor surface required by the
    current backend.
    """

    if "capabilities" not in hello:
        return None
    ...
    for item in raw:
        if not isinstance(item, str):
            raise ValueError("invalid_capabilities")
        value = item.strip()
        if not value or len(value) > MAX_CAPABILITY_CHARS:
            raise ValueError("invalid_capabilities")
...
async def _serve(websocket: WebSocket):
    ...
    manager = runtime.extension_manager
    ...
    try:
        ...
        if prior:
            ...
            try:
                await prior.ws.close(4000, "superseded_by_new_connection")
            except Exception:
                pass
            bridge.clear(connection_id=prior.id)

        conn = bridge.register(adapter, hello)
        manager.connected(conn)
        runtime.wake_worker()
        await bridge.handle_message(hello, adapter)
        ...
        try:
            await websocket.close(1011, "extension gateway failure")
        except Exception:
            pass
    finally:
        if heartbeat_task:
