...
@router.get("")
def list_extensions(request: Request,_client=Depends(get_client)):
    return {"object":"list","data":_manager(request).list(),"has_more":False,"next_cursor":None}


@router.get("/{installation_id}")
def get_extension(installation_id: str,request: Request,_client=Depends(get_client)):
    return _require_known(request,installation_id)


@router.post("/{installation_id}/pause")
def pause_extension(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
    return _manager(request).pause(installation_id)


@router.post("/{installation_id}/resume")
def resume_extension(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
    return _manager(request).resume(installation_id)


@router.post("/{installation_id}/ping")
async def ping_extension(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
...
async def refresh_extension_auth(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
...
async def open_flow(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
...
async def reconnect_extension(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
...
async def extension_diagnostics(installation_id: str,request: Request,_client=Depends(get_client)):
    _require_known(request,installation_id)
