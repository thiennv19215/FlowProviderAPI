...
def _validate_upload_request(request: Request,payload: AssetUploadRequest)->None:
    ...
    if payload.size_bytes is not None and payload.size_bytes>limit:raise APIError(413,"ASSET_TOO_LARGE",f"Asset exceeds the {limit} byte upload limit.",error_type="validation_error",param="size_bytes")
    expected="image/" if payload.type=="image" else "video/"
    if not payload.content_type.lower().startswith(expected):raise APIError(422,"INVALID_ASSET_CONTENT_TYPE",f"Asset type '{payload.type}' requires a {expected}* content type.",error_type="validation_error",param="content_type")
    ...
    try:
        with tempfile.NamedTemporaryFile(prefix="flow-provider-upload-",delete=False) as tmp:
            ...
            async for chunk in request.stream():
                ...
                tmp.write(chunk)
        if asset.size_bytes is not None and asset.size_bytes!=size:raise APIError(409,"UPLOAD_SIZE_MISMATCH",f"Uploaded {size} bytes but {asset.size_bytes} bytes were declared.",error_type="conflict_error")
