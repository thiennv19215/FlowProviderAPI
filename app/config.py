...
class Settings(BaseSettings):
    ...
    # Dev/test can use the filesystem. Production media is Provider-owned and
    # stored durably in Cloudflare R2 so account/project failover can re-upload
    # references without asking callers to resend source bytes.
    ...
    r2_region: str = "auto"
    r2_download_url_expires_seconds: int = Field(default=900, ge=60, le=86400)
    ...
    account_unavailable_retry_seconds: int = Field(default=30, ge=5, le=600)
    ...
    def validate_settings(self):
        ...
        if self.storage_backend == "r2":
            missing = [
                name
                for name, value in {
                    "R2 endpoint URL": self.r2_endpoint_url,
                    "R2 access key ID": self.r2_access_key_id,
                    "R2 secret access key": self.r2_secret_access_key,
                    "R2 bucket": self.r2_bucket,
                }.items()
                if not value
            ]
            if missing:
                raise ValueError(f"R2 storage requires: {', '.join(missing)}")
            parsed_r2 = urlparse(self.r2_endpoint_url or "")
            if self.env == "production" and parsed_r2.scheme != "https":
                raise ValueError("Production R2 endpoint URL must use HTTPS")
        if self.env == "production":
            if not self.database_url.startswith("postgresql"):
                raise ValueError("Production requires PostgreSQL")
            if self.storage_backend != "r2":
                raise ValueError("Production requires Provider-owned R2 storage")
            if self.bootstrap_api_key and (
                self.bootstrap_api_key.startswith("fpa_dev_")
                or "change_me" in self.bootstrap_api_key.lower()
            ):
