...
The API resolves the owning account/project and media references, persists a `queued` job, and returns its Provider job ID. `JobWorker` atomically claims both image and video work. Image generation completes in that one worker call and is never added to the upstream polling schedule. Video dispatch records a Flow poll identifier and the worker polls it with a durable schedule (`next_poll_at`, attempts, consecutive errors and latest error). `/v1/jobs/status` and the compatible `/v1/videos/status` read durable rows only and never consume a live extension slot.
