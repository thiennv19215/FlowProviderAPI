...
Every HTTP response has `X-Request-Id`, matching `error.request_id` on a synchronous error response. Send an existing value in `X-Request-Id` to correlate calls across services. Authentication errors include `WWW-Authenticate: Bearer`. API rate-limit errors may include `Retry-After`; task polling does not use that header.
