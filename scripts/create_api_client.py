...
settings=get_settings();engine=build_engine(settings.database_url)
if "api_clients" not in inspect(engine).get_table_names():
    raise SystemExit("Database is not migrated. Run: alembic upgrade head")
Session=build_session_factory(engine);api_key="fpa_live_"+secrets.token_urlsafe(32)
with Session() as db:
