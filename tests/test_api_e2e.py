...
def test_video_dispatch_and_poll_survives_db_state(client, app, auth):
    ...
    response = client.post(
        ...
        json={
            "prompt": "move",
            "provider": "fake",
            "start_media_id": asset_id,
            "workspace": {"key": "test:video:1"},
        },
    )
    job_id = response.json()["task_id"]
    assert asyncio.run(app.state.runtime.worker.run_once()) is True
    mid = client.get(f"/v1/status/{job_id}", headers=auth).json()
...
def test_duplicate_submissions_always_create_new_tasks(client, auth):
    payload = {
        "prompt": "cat",
        "provider": "fake",
        "workspace": {"key": "duplicate:1"},
    }
    headers = {**auth, "Idempotency-Key": "ignored-order-123"}
...
def test_google_flow_generation_fails_immediately_without_online_account(
    client, app, auth
):
    response = client.post(
        "/v1/images/generations",
...
def test_google_flow_generation_is_accepted_while_connected_account_syncs(
    client, app, auth
):
    app.state.runtime.bridge.register(
        object(),
        {
...
def test_new_media_ids_are_strings_and_url_safe(client, auth):
    response = client.post(
        "/v1/media",
        headers=auth,
        files={"file": ("ref.png", b"image", "image/png")},
    )
...
def test_structured_auth_error(client):
    response = client.get("/v1/status/nope")
...
def test_validation_error_reports_all_fields(client, auth):
    response = client.post(
        "/v1/images/generations",
        headers=auth,
        json={"prompt": "", "model": "unknown", "output_count": 9},
    )
    ...
    assert details == {
        "prompt": "INVALID_LENGTH",
        "model": "INVALID_CHOICE",
        "output_count": "OUT_OF_RANGE",
    }
...
def test_malformed_json_and_unknown_routes_use_same_error_contract(client, auth):
    ...
    assert (
        set(missing.json()["error"])
        - {"status_code", "code", "message", "details", "request_id", "retryable"}
        == set()
    )
...
def test_openapi_exposes_typed_generation_and_asset_responses(client):
    ...
    assert set(
        schema["components"]["schemas"]["ImageToVideoRequest"]["properties"]
    ) == {"prompt", "start_media_id", "quality", "aspect_ratio"}
    image = schema["paths"]["/v1/images/generations"]["post"]
    assert image["responses"]["202"]["content"]["application/json"]["schema"][
        "$ref"
    ].endswith("/JobOutput")
    assert image["responses"]["422"]["content"]["application/json"]["schema"][
        "$ref"
    ...
    assert "workspace" not in schema["components"]["schemas"]["ImageGenerationRequest"][
        "properties"
    ]
    assert "task_id" in schema["components"]["schemas"]["JobOutput"]["properties"]
    assert "workspace_key" not in schema["components"]["schemas"]["JobOutput"][
        "properties"
    ]
    assert set(schema["components"]["schemas"]["JobOutput"]["properties"]) == {
        "task_id",
        "status",
        "outputs",
        "error",
    ...
    assert set(
        schema["components"]["schemas"]["ImageGenerationRequest"]["properties"]
    ) == {"prompt", "model", "aspect_ratio", "output_count", "reference_media_ids"}
    model_schema = schema["components"]["schemas"]["ImageGenerationRequest"][
        "properties"
    ]["model"]
    assert model_schema["enum"] == ["banana_pro", "banana_2"]
    assert model_schema["default"] == "banana_pro"
    assert schema["components"]["schemas"]["ImageGenerationRequest"]["properties"][
        "aspect_ratio"
    ]["default"] == "9:16"
    upload = schema["paths"]["/v1/media"]["post"]
    assert upload["responses"]["201"]["content"]["application/json"]["schema"][
        "$ref"
    ].endswith("/MediaOutput")
    ...
    status_list = schema["paths"]["/v1/status"]["get"]
    params = {parameter["name"]: parameter for parameter in status_list["parameters"]}
    ...
    assert params["type"]["schema"]["anyOf"][0]["enum"] == [
        "image",
        "video",
        "omni",
    ]
...
def test_legacy_media_upload_routes_are_removed(client, auth):
    ...
    assert (
        client.post("/v1/media/media_unused/complete", headers=auth).status_code == 404
    )
...
def test_media_upload_body_is_limited_before_multipart_parsing(client, app, auth):
    ...
    response = client.post(
        "/v1/media",
        headers=auth,
        files={"file": ("too-large.png", b"x" * (2 * 1024 * 1024), "image/png")},
    )
